...
const PERM = {
  ...
  handover_protocol_write: ["sofer", "dispecer_pozicovne", "veduci_pozicovne"],
...
function DispatcherApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [m, oldDrivers, j, oldTechnicians, a, dmg, wd, notif, tsl, cust, dc, fz, bl, cs, res, emp, plogs, hprot] = await Promise.all([
        loadKey("machines", []),
        loadKey("drivers", []),
        loadKey("jobs", []),
        loadKey("technicians", []),
    ...
    return (
      ...
      >
        ...
        <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
          ...
          {can(user, "handover_protocol_write") && (
            <div>
              ...
              {(() => {
                ...
                const label = !hp ? "Bez protokolu" : !hp.returnDone ? "Čaká na vrátenie" : "Hotovo";
                const color = !hp ? "var(--text-dim)" : !hp.returnDone ? "var(--warn)" : "var(--ok)";
                return (
                  <button
                    className="btn btn-ghost"
                    style={{ fontSize: 11, padding: "3px 8px", color, borderColor: color }}
...
function HandoverProtocolViewPanel({ existing, user, onGoEditNextPhase, onRequestCorrection }) {
    ...
    <div>
      ...
      {confirmingCorrection ? (
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {!existing.returnDone && can(user, "handover_protocol_write") && (
...
function HandoverProtocolModal({ job, machine, existing, user, onClose, onSave, onDelete, canDelete }) {
...
function JobDetailModal({ job, machine, driverById, technicianById, depoCheckers, checkerSubstitutions, salespeople, handoverProtocol, user, onClose, onEdit, onComplete, onUncomplete, onReportDamage, onOpenHandoverProtocol }) {
  ...
  return (
    <Modal title={`${machine?.code || "—"}${machine?.type ? " · " + machine.type : ""}`} onClose={onClose}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        {machine && can(user, "handover_protocol_write") && (
          <button className="btn btn-ghost" onClick={onOpenHandoverProtocol}>
            {!handoverProtocol && "📋 Protokol o odovzdaní"}
            {handoverProtocol && !handoverProtocol.returnDone && "📋 Dokončiť vrátenie"}
            {handoverProtocol?.returnDone && "📋 Zobraziť protokol"}
          </button>
        )}
      </div>
    </Modal>
...
--------------------------------------------------------- */
function MachineCardModal({ machine, history, jobs, handoverProtocols, protocolLogs, user, onClose, onReportDamage, onAddJob, onAddReservation, onEditJob, onCompleteJob, onOpenDamage, onArchive, onUnarchive, onDelete, onToggleTrackRevisions, onToggleTrackUradnaSkuska, onEditMachine, onOpenHandoverProtocol }) {
  const m = machine;
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {jobs
              ...
              .map((j) => {
                ...
                return (
                  <div key={j.id} className="panel" style={{ padding: 10, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                    ...
                    {can(user, "handover_protocol_write") && (
                      <button
                        className="btn btn-ghost"
                        style={{ fontSize: 11, padding: "3px 8px", color: badgeColor, borderColor: badgeColor }}
                        onClick={() => onOpenHandoverProtocol(j)}
                      >
                        {badgeLabel}
                      </button>
                    )}
