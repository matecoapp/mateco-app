...
function DispatcherApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [m, oldDrivers, jobsTable, oldTechnicians, a, dmg, wd, notif, tsl, cust, dc, fz, bl, cs, res, emp, plogs, hprot] = await Promise.all([
...
function ProtocolModal({ html, params, onClose }) {
  ...
  return (
    ...
    >
      ...
      <div className="protocol-frame-wrap" style={{ flex: 1, background: "#fff", borderRadius: 8, overflow: "hidden" }}>
        <iframe
          title="Servisný protokol"
          srcDoc={html}
          sandbox="allow-scripts allow-forms allow-modals allow-downloads allow-same-origin"
          style={{ width: "100%", height: "100%", border: "none", overscrollBehavior: "none", touchAction: "pan-y" }}
        />
      </div>
    </div>
  );
      ...
      >
        ...
        <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
          ...
          {(() => {
            ...
            const nextPhase = !hp ? "prevzatie" : !hp.returnDone ? "vratenie" : null;
            const isMyDriverAction = nextPhase && can(user, "handover_protocol_write") && !can(user, "handover_protocol_edit_locked");
            const eligible = !isMyDriverAction || canFillHandoverPhase(fullJob, myEmployee, nextPhase);
            ...
            const actionLabel = !hp ? "📋 Vypísať protokol" : !hp.returnDone ? "📋 Dokončiť vrátenie" : "📋 Zobraziť";
            return (
              <div>
...
function AddMachineModal({ existing, onClose, onSave }) {
  ...
  const [trackUtilization, setTrackUtilization] = useState(
    existing ? existing.trackUtilization !== false : true
  );
  return (
    <Modal title={existing ? "Upraviť údaje stroja" : "Pridať stroj"} onClose={onClose}>
      ...
      <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, marginBottom: 14, cursor: "pointer" }}>
        <input type="checkbox" checked={trackUtilization} onChange={(e) => setTrackUtilization(e.target.checked)} />
        Zahrnúť do štatistík utilizácie
      </label>
      <button
        className="btn btn-accent"
        disabled={!code.trim() || !type.trim() || !depo.trim()}
        onClick={() => onSave({ code: code.trim(), type: type.trim(), depo: depo.trim(), revizia, uradnaSkuska: uradnaSkuska.trim(), trackUtilization })}
      >
        {existing ? "Uložiť zmeny" : "Uložiť"}
      </button>
    </Modal>
...
function ImportModal({ onClose, onImport }) {
  ...
  function doImport() {
    const machines = rows
      ...
      .filter((m) => m.code)
      .map((m) => ({ id: uid(), ...m, trackUtilization: !m.code.toUpperCase().startsWith("EXT") }));
    onImport(machines);
  }
...
function PoziciovnaStatistiky({ machines, jobs, reservations, today, start, end, canEdit, showExclusions, setShowExclusions, onUpdateMachine }) {
  const [machineFilter, setMachineFilter] = useState("");
  const active = machines.filter((m) => !m.archived);
  const tracked = active.filter((m) => m.trackUtilization !== false);
  ...
  const filteredMachines = machineFilter.trim()
    ? active.filter((m) => `${m.code} ${m.type}`.toLowerCase().includes(machineFilter.trim().toLowerCase()))
    : active;
      ...
      {canEdit && (
        <div>
          <button className="btn btn-ghost" onClick={() => setShowExclusions((v) => !v)}>
            {showExclusions ? "Skryť" : "Upraviť"} zoznam strojov v štatistikách
          </button>
          {showExclusions && (
            <div style={{ marginTop: 10 }}>
              <input
                type="text"
                placeholder="Hľadať podľa sériového čísla alebo modelu, napr. EXT..."
                value={machineFilter}
                onChange={(e) => setMachineFilter(e.target.value)}
                style={{ width: "100%", marginBottom: 8 }}
              />
              <div className="panel" style={{ padding: 0, overflow: "hidden", maxHeight: 400, overflowY: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {filteredMachines.length === 0 ? (
                      <tr>
                        <td colSpan={2} style={{ padding: 16, fontSize: 13, color: "var(--text-dim)" }}>Žiadny stroj nezodpovedá hľadaniu.</td>
                      </tr>
                    ) : (
                      filteredMachines.map((m) => (
                        <tr key={m.id} style={{ borderTop: "1px solid var(--border)" }}>
                          <td style={{ padding: "6px 12px", fontSize: 13 }}>{m.code} {m.type ? `· ${m.type}` : ""}</td>
                          <td style={{ padding: "6px 12px" }}>
                            <input
                              type="checkbox"
                              checked={m.trackUtilization !== false}
                              onChange={(e) => onUpdateMachine(m.id, { trackUtilization: e.target.checked })}
                            />
                          </td>
                        </tr>
                      ))
