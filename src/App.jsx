...
/* ---------------------------------------------------------
   Zákazky — samostatná tabuľka "jobs", jeden riadok na zákazku.
   Namiesto ukladania celého zoznamu naraz sa ukladá a maže po jednotlivých
   riadkoch, aby dvaja ľudia mohli meniť rôzne zákazky súčasne bez rizika,
   že si navzájom prepíšu zmeny.
--------------------------------------------------------- */
async function loadJobsTable() {
  try {
    ...
    console.error("loadJobsTable failed", e);
    return [];
  }
}
const _jobSaveQueues = {};
async function writeJobRowWithRetry(job, attempt = 1) {
  try {
    ...
    if (attempt < 4) {
      await sleep(attempt * 800);
      return writeJobRowWithRetry(job, attempt + 1);
    }
    console.error("Job row save failed after retries", job.id, e);
    return false;
  }
}
function saveJobRow(job) {
  const prev = _jobSaveQueues[job.id] || Promise.resolve();
  const next = prev.then(async () => {
    _saveStatusListener?.("pending", "jobs");
    const ok = await writeJobRowWithRetry(job);
    _saveStatusListener?.(ok ? "ok" : "error", "jobs");
  });
  _jobSaveQueues[job.id] = next;
  return next;
}
async function deleteJobRow(id) {
  const prev = _jobSaveQueues[id] || Promise.resolve();
  const next = prev.then(async () => {
    try {
      await supabase.from("jobs").delete().eq("id", id);
    } catch (e) {
      console.error("deleteJobRow failed", id, e);
    }
  });
  _jobSaveQueues[id] = next;
  return next;
}
...
function DispatcherApp() {
  ...
  // Zákazky majú vlastnú, samostatnú synchronizáciu — počúva sa na zmenu
  // JEDNOTLIVÝCH riadkov v tabuľke "jobs", nie na zmenu celého zoznamu naraz.
  // Vďaka tomu sa pri zmene od kolegu aktualizuje v appke len tá jedna
  // konkrétna zákazka, ostatné zostanú nedotknuté.
  useEffect(() => {
    if (!session) return;
    const channel = supabase
      .channel("jobs_live")
  ...
  const persistJobs = useCallback((next) => {
    setJobs((prev) => {
      ...
      for (const id of prevById.keys()) {
        if (!nextIds.has(id)) deleteJobRow(id);
      }
      for (const j of next) {
        const before = prevById.get(j.id);
        if (!before || JSON.stringify(before) !== JSON.stringify(j)) {
          saveJobRow(j);
        }
      }
      return next;
  ...
  const persistHandoverProtocols = useCallback((next) => {
    setHandoverProtocols(next);
    saveKey("handoverProtocols", next);
  }, []);
